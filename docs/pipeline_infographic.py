...
def draw_stage_header(ax, y, stage_num, title, color):
    circle = plt.Circle(
        (1.5, y), 0.6, facecolor=color, edgecolor="white", linewidth=2, zorder=3
    )
    ax.add_patch(circle)
    ax.text(
        1.5,
    ...
    ax.plot(
        [2.5, 18.5], [y - 0.3, y - 0.3], color=color, linewidth=2, alpha=0.5, zorder=1
    )
...
ax.text(
    10,
    26.2,
    "Multi-Stage Character Encoding Detection Pipeline",
    ha="center",
    va="top",
    fontsize=16,
    color=colors["text"],
...
ax.text(
    10,
    25.7,
    "Universal Charset Detection with Progressive Analysis",
    ha="center",
    va="top",
    fontsize=11,
    color=colors["subtext"],
...
for i, metric in enumerate(struct_metrics):
    ax.text(
        11, 16.6 - i * 0.3, f"> {metric}", fontsize=7, color=colors["subtext"], va="top"
    )
...
stat_details = [
    "65536 bigram entries",
    "Weighted profile building",
    "Language-specific models",
]
for i, detail in enumerate(stat_details):
    ax.text(
        3, 12.3 - i * 0.25, f"* {detail}", fontsize=6, color=colors["subtext"], va="top"
    )

box_simplified = draw_box(
    ax,
    7,
    12,
    3.5,
...
for i in range(len(stages_y) - 1):
    ax.annotate(
        "", xy=(10, stages_y[i + 1]), xytext=(10, stages_y[i]), arrowprops=arrow_style
    )

exit_style = dict(
    arrowstyle="->", color=colors["success"], lw=1.5, mutation_scale=12, linestyle="--"
)
for x in [3.75, 7.75, 11.75, 16]:
    ax.annotate(
        "",
        xy=(19, 22.5),
        xytext=(x + 1.5 if x < 15 else x + 2, 22.25),
...
for i, (color, label) in enumerate(legend_items):
    ...
    rect = plt.Rectangle(
        (x_pos, y_pos - 0.1), 0.4, 0.25, facecolor=color, edgecolor="white", linewidth=1
    )
    ax.add_patch(rect)
    ax.text(
        x_pos + 0.6,
...
ax.text(
    10,
    0.5,
    "Chardet-Rust - Universal Character Encoding Detection - Rust + PyO3",
    ha="center",
    va="center",
    fontsize=9,
    color=colors["subtext"],
